...
def get_parser() -> ArgumentParser:
    ...
    parser.add_argument(
        "--valset_split", type=float, default=0.1, help="bias injector."
    )
...
class BiC(ContinualModel):
    ...
    def __init__(self, backbone, loss, dataset_config, args, transform):
        super().__init__(backbone, loss, dataset_config, args, transform)

        dd = get_dataset(args)
        self.n_tasks = dd.NUM_CLASSES // args.classes_per_task
        ...
        with torch.no_grad():
            with bn_track_stats(self, False):
                for data in self.val_loader:
                    ...
                    resp += self.forward(inputs, anticipate=fprefx == "post")[
    ...
    def end_task(self, dataset):
        if self.task > 0:
            ...
            corr_factors = torch.tensor(
                [0.0, 1.0], device=self.device, requires_grad=True
            )
            ...
            for l in range(self.args.bic_epochs):
                for data in self.val_loader:
                    ...
                    tout = out + 0
                    tout[:, start_last_task:end_last_task] *= corr_factors[
                        1
                    ].repeat_interleave(end_last_task - start_last_task)
                    tout[:, start_last_task:end_last_task] += corr_factors[
                        0
                    ].repeat_interleave(end_last_task - start_last_task)
    ...
    def forward(self, x, anticipate=False):
        ...
        if ret.shape[0] > 0:
            if hasattr(self, "corr_factors"):
                ...
                ret[:, start_last_task:end_last_task] *= self.corr_factors[
                    1
                ].repeat_interleave(end_last_task - start_last_task)
                ret[:, start_last_task:end_last_task] += self.corr_factors[
                    0
    ...
    def observe(self, inputs, labels, not_aug_inputs):
        ...
        if self.task > 0:
            with torch.no_grad():
                ...
                if self.args.distill_after_bic:
                    if hasattr(self, "old_corr"):
                        ...
                        old_outputs[:, start_last_task:end_last_task] *= self.old_corr[
                            1
                        ].repeat_interleave(end_last_task - start_last_task)
                        old_outputs[:, start_last_task:end_last_task] += self.old_corr[
                            0
            ...
            pi_hat = F.log_softmax(
                outputs[:, : self.task * self.cpt] / self.args.temp, dim=1
            )
            pi = F.softmax(
                old_outputs[:, : self.task * self.cpt] / self.args.temp, dim=1
            )

            dist_loss = -(pi_hat * pi).sum(1).mean()

        class_loss = self.loss(
            outputs[:, : (self.task + 1) * self.cpt], labels, reduction="none"
        )
        loss = (
            1 - self.lamda
        ) * class_loss.mean() + self.lamda * dist_loss.mean() * self.args.temp * self.args.temp
        ...
        if self.task > 1:
            ...
            for ttl in buf_tl.unique():
                ...
                self.buffer.add_data(
                    examples=ex[:first], labels=lab[:first], task_labels=tasklab[:first]
                )
            ...
            for i, data in enumerate(dataset.train_loader):
                ...
                if examples_per_task - counter > 0:
                    self.buffer.add_data(
                        ...
                        task_labels=(
                            torch.ones(self.args.batch_size) * (self.task - 1)
