...
def get_parser() -> ArgumentParser:
    parser = ArgumentParser(description="Continual learning via X-DER.")
...
class XDer(ContinualModel):
    ...
    def __init__(self, backbone, loss, dataset_config, args, transform):
        super(XDer, self).__init__(backbone, loss, dataset_config, args, transform)
        self.buffer = Buffer(self.args.buffer_size, self.device)
        self.cpt = args.classes_per_task
        self.tasks = get_dataset(args).NUM_CLASSES // args.classes_per_task
        self.task = 0
        ...
        self.dataset_shape = (
            get_dataset(args).get_data_loaders()[0].dataset.data.shape[2]
        )
        self.gpu_augmentation = strong_aug(
            self.dataset_shape, self.dataset_mean, self.dataset_std
        )
        self.simclr_lss = SupConLoss(
            temperature=self.args.simclr_temp,
            base_temperature=self.args.simclr_temp,
            reduction="sum",
        )
    ...
    def end_task(self, dataset):
        ...
        if self.args.start_from is None or self.task >= self.args.start_from:
            ...
            if self.task > 0:
                examples_per_class = self.args.buffer_size // (
                    (self.task + 1) * self.cpt
                )
            ...
            with torch.no_grad():
                with bn_track_stats(self, False):
                    if (
                        self.args.start_from is None
                        or self.args.start_from <= self.task
                    ):
                        for data in dataset.train_loader:
                            ...
                            # Update past logits
                            if self.task > 0:
                                outputs = self.update_logits(
                                    outputs, outputs, labels, 0, self.task
                                )
                    ...
                    if chosen.any():
                        ...
                        self.buffer.logits[buf_idx[chosen], :] = to_transplant.to(
                            self.buffer.device
                        )
    ...
    def observe(self, inputs, labels, not_aug_inputs):
        ...
        # Present head
        loss_stream = self.loss(
            outputs[:, self.task * self.cpt : (self.task + 1) * self.cpt],
            labels % self.cpt,
        )

        loss_der, loss_derpp = torch.tensor(0.0), torch.tensor(0.0)
        if not self.buffer.is_empty():
            ...
            if self.args.take_one_batch_from_buffer:
                buf_idx2, buf_inputs2, buf_labels2, buf_logits2, buf_tl2 = (
                    torch.clone(buf_idx1),
                    torch.clone(buf_inputs1),
                    torch.clone(buf_labels1),
                    torch.clone(buf_logits1),
                    torch.clone(buf_tl1),
                )
            else:
                (
            ...
            with torch.no_grad():
                ...
                c = chosen.clone()
                chosen[c] = (
                    torch.rand_like(chosen[c].float()) * self.update_counter[buf_idx[c]]
                    < 1
                )
        ...
        if self.task < self.tasks - 1:
            ...
            if not self.buffer.is_empty():
                if self.args.take_one_batch_from_buffer:
                    buf_idxscl, buf_na_inputsscl, buf_labelsscl, buf_logitsscl, _ = (
                        torch.clone(buf_idx1),
                        torch.clone(buf_inputs1),
                        torch.clone(buf_labels1),
                        torch.clone(buf_logits1),
                    ...
                    (
                        ...
                        self.args.simclr_batch_size, transform=None, return_index=True
                    )
                scl_na_inputs = torch.cat([buf_na_inputsscl, scl_na_inputs])
                scl_labels = torch.cat([buf_labelsscl, scl_labels])
            with torch.no_grad():
                scl_inputs = self.gpu_augmentation(
                    scl_na_inputs.repeat_interleave(self.args.simclr_num_aug, 0)
                ).to(self.device)

            with bn_track_stats(self, False):
                scl_outputs = self.net(scl_inputs).float()

            scl_featuresFull = scl_outputs.reshape(
                -1, self.args.simclr_num_aug, scl_outputs.shape[-1]
            )  # [N, n_aug, 100]

            scl_features = scl_featuresFull[
                :, :, (self.task + 1) * self.cpt :
            ]  # [N, n_aug, 70]
            scl_n_heads = self.tasks - self.task - 1

            scl_features = torch.stack(
                scl_features.split(self.cpt, 2), 1
            )  # [N, 7, n_aug, 10]

            loss_cons = torch.stack(
                [
                    self.simclr_lss(
                        features=F.normalize(scl_features[:, h], dim=2),
                        labels=scl_labels,
                    )
                    for h in range(scl_n_heads)
                ]
            ).sum()
        ...
        loss_constr_past = torch.tensor(0.0).type(loss_stream.dtype)
        if self.task > 0:
            ...
            loss_constr = (
                bad_head.max(1)[0].detach() + self.args.m - good_head.max(1)[0]
            )
        ...
        loss_constr_futu = torch.tensor(0.0)
        if self.task < self.tasks - 1:
            ...
            if not self.buffer.is_empty():
                ...
                bad_head = torch.cat(
                    [bad_head, buf_outputs[:, (self.task + 1) * self.cpt :]]
                )
                good_head = torch.cat(
                    [
                        good_head,
                        torch.stack(buf_outputs.split(self.cpt, 1), 1)[
                            torch.arange(len(buf_tlgt)), buf_tlgt
                        ],
                    ]
                )
