...
def fill_buffer(self, mem_buffer: Buffer, dataset, t_idx: int) -> None:
    ...
    classes_start, classes_end = (
        t_idx * self.args.classes_per_task,
        (t_idx + 1) * self.args.classes_per_task,
    )
...
class ICarl(ContinualModel):
    ...
    def __init__(self, backbone, loss, dataset_config, args, transform):
        super(ICarl, self).__init__(backbone, loss, dataset_config, args, transform)
        self.dataset = get_dataset(args)

        # Instantiate buffers
        self.buffer = Buffer(self.args.buffer_size, self.device)
        self.eye = torch.eye(self.dataset.NUM_CLASSES).to(self.device)
    ...
    def get_loss(
        ...
        pc = task_idx * self.args.classes_per_task
        ac = (task_idx + 1) * self.args.classes_per_task
